...
### 8.2 与 `stall_guard.min_tps` 的关系（熔断 ≠ 评级）

min_tps 是**熔断兜底**（防"接近死机"白烧长跑），不是 UX 评级——两者机制不同：
UX 评级是绝对值（阅读速度不随 GPU 变），退化检测是相对值（H100 与 4090 的健康单流
速度差一个量级，固定高阈值在慢机器上要么永远误触发要么形同虚设）。因此：

- `min_tps` 默认 **10**（低于任何机器的健康单流速度、高于死机趋 0 状态；12.10 起判据
  即单流 decode 速度中位——单用户体验口径，聚合吞吐只作归因诊断、不进熔断）；
- 部署级建议值由 `bench probe` 自动实测（`decode_speed` 检查项，2 次取保守值），
  换部署重跑一次 probe 即可，无需手工标定；
- "退化与否"的主力判定是 8.2 恢复探针的相对机制（`probe_factor × min_tps`，两者同为单流口径）。

### 8.3 口径三条

- **output speed 只计首 token 之后**（与 TPOT/ITL、Artificial Analysis methodology 一致，不含 TTFT）；
- **聚合 = 多请求求和**（吞吐观测口径：并发下排队不算慢，单请求速度不背锅；注——stall_guard
  熔断自 12.10 起改按单流速度中位判定，与本条观测口径已不同轨）；
