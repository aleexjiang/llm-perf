...
> 当前无在途项——第 10 节体系收敛随 10.5 落地全部完成（2026-09-16）。
> （5.9 起步值补样移出在途清单、不再跟踪——首轮取证结论已够用。）
> 新增（2026-09-15）：**第 12 节 自部署实测复盘**——qwen3.8-27b 双环境四轮实测中工具硌手点清单（12.1–12.13 已全部落地）。
> 已完成（2026-09-16）：10.5 稳定性 soak（`duration_seconds` + `renew` 时长制原语 + 报告稳定性区）。
...
### 12.10 stall_guard 判据从聚合改为单流【已实现，2026-09-16】

- 现象（r1-off S5 实测）：并发多轮爬坡到 20 路，单请求 decode 已劣化到 6–16 tok/s（健康基线 25），但熔断全程未触发。原因：`stall_guard` 判据是**聚合输出速度**，饱和段聚合仍有 ~113 tok/s，远高于 min_tps=24；偶发跌破只持续数秒，回升即重置计数。
- **拍板（2026-09-16，用户）**：「熔断应该按照单流的值来判断」——单流 decode 劣化到不可用就是该停的信号（客户端体验口径），不再以聚合吞吐为准。**本条取代第 7 节的聚合判据拍板**；聚合值保留为 stall.csv 观测面，不作为熔断依据。
- 实现要点：
  - `stall.go` 由全局 token 计数改为 per-stream 记账（每在飞流独立计窗口内输出 token 与 decode 活跃时长）；
  - 判据取在飞流的**单流 decode 速度中位数**（2026-09-16 拍板口径：抗单流偶发抖动，反映普遍劣化）；流需已出首 token（排除 TTFT 段）；
  - `min_tps` 语义变为单流阈值（现有配置 24 语义自动对齐；r1 实测口径取 20——用户口头判据「掉到 20 以下就该停」）；
  - 触发后行为链不变（中止当前场景 → 冷却 → 恢复探针）；stall.csv / 报告 note 标注 rate 口径已换为单流；
...
### 12.13 沙盘推演（采集/计算全链路审查）修复【已实现，2026-09-16】

- 方式：全链路只读走查（cmd → engine → scenario → smetrics → report → gen_html_report.py）+ mock 实测重算（全部派生指标误差 <0.05ms）+ 静态分析三件套（deadcode / pyflakes / vulture）；证据全文 `docs/sandbox-review-2026-09-16.md`。
- 修复（全部附回归断言）：
  - 单发 TTFT 图未剔失败 run（失败无 ttft、按 0 计入 → 图中位被拉低，与全文件其余统计口径不一致）→ `single_ttft_median()` 统一同口径（fixtures I 组）；
  - 恢复探针模型选择用精确相等、与 `-m` 子串语义不一致（子串命中时探针探空模型 → 误判「服务端未恢复」并停剩余场景）→ `scenario.FirstMatchedModel`（单测）；
  - plan 画像未含时长制（runs_per_worker 被场景层忽略、画像仍按其估算）→ `duration_s` 标记 + 请求数不估算（渲染层「—」；单测 + smoke）；
  - 时长制 soak + renew 下「发车」span_s 被续跑会话撑到整场时长（整场 soak 误标暂态窗）→ 改取各批次**首次**启动时刻差（fixtures I 组）。
- 死代码清理：Python 1 死函数（`axis_lbl`）+ 5 死变量（含级联 `t_kind`）+ 3 未用 import + 2 夹具死变量；Go 侧零死代码（deadcode 复核）。data-contract 补 `server_counter_delta` 语义（指标细则 14）。
...
**5.7 负载保真度：闭环错峰发车与会话续跑**（爬坡发车已实现 2026-09-11；会话续跑 filler 侧已随 10.5 落地 2026-09-16）
...
- 会话续跑（filler 侧已随 10.5 落地，见第 10 节；trace 回放侧仍暂缓，见文末附表）：✅
...
**5.11 混合档：跨会话混合增量档（multiturn.profiles）**【已实现，2026-09-16】

- 问题：并发多轮此前全部会话同一增量形状——均匀轻量档测不到"重度会话抢 KV/算力拖累轻量会话体验"，
  均匀重度档又把尾巴当常态。AgentLens 复核（n=213）：每用户轮增量 p50 +4.0k / p90 +23.5k，
  重尾真实存在（20k+/轮占 12%）但非常态 → 需要"轻量占多数 + 重度占少数同场跑"的机制。
- 实现 ✅：`multiturn.profiles`（name/weight/turn_tokens/turns，turns=0 继承全局）——并发多轮按权重
  把会话分属不同增量档；SWRR 平滑加权轮转确定性分配（与 5.6 concurrent.mix 同源，重跑一致）；
  `report.MultiturnRun.profile` 档位标签（报告/分析按档分组切各档位体验）；plan 画像逐会话求和
  （精确到档位轮数之和，均匀档退化为 level×turns 不变）。
- 边界口径：仅并发多轮生效（单发多轮忽略并提示）；标量 turn_tokens 在并发多轮下让位于档位（并存告警）；
  与 `dataset.mode=trace` 互斥（重放形状来自数据、无法按权重分档）；`concurrent.mix × multiturn`
  互斥错误语指向 profiles。
- 验证：config 5 态拒绝 + trace 互斥单测；scenario 单测（SWRR 分布 3:1 周期、逐会话求和、端到端
  分档执行带标签）；plan 单测（档位口径请求量）；example.yaml 示例 + data-contract 契约更新。
...
5.7 会话续跑的 filler 侧已提级为 10.5（第 10 节）并已落地；5.5 与 trace 侧续跑为暂缓项，见文末附表。
2026-09-16 更新：5.9 补样移出在途清单、不再跟踪；5.11 混合档（跨会话 profiles）已实现。）
...
- **做法**：`stall_guard` 按**聚合输出速度**判定——窗口内所有在飞请求的输出 token 之和 / 窗口时长
  ...
  ⚠️ **判据口径已被 12.10 取代（2026-09-16 拍板：改按单流值判定）**——聚合口径的原始理由
  「单请求慢可能只是排队」在容量爬坡场景恰好是要抓的拐点信号；本节保留作历史记录。
...
**10.5 稳定性 soak（filler 先行）**【已实现，2026-09-16】

- `concurrent.duration_seconds` + `renew` 时长制原语【已实现】：跑满墙钟（`runs_per_worker` 忽略）；
  `renew: true` 时会话滚完 `turns` 轮换新 seed 重开（序号 `workerID + sr*level` 全局唯一、上下文清零重涨），
  暂态后在途会话年龄铺满 0~turns = 稳态。校验取严格口径：开环 / trace 模式 / 多轮未开 renew 均 fail-fast。
  hook 到点 `capped()` 停发新请求（drain 语义，在途跑完）
- 报告稳定性区【已实现】：漂移分析（首/末时段各 25% 时间窗；renew 剔暂态——单会话时长中位数上限半程防吃光窗口；
  goodput 掉 >15% 判退化 ❌，未配 slo.goodput 时只陈述数字不定性）、事故时间轴（扫 `*.stall.csv` 摘熔断时刻，纯报告侧）；
  取不到如实写 NA
- smoke/fixtures【已实现】：`configs/smoke-soak.yaml`（duration 12s + renew）断言时长制落盘、续跑 >1 次、起始偏移递增；
  fixtures H 组五态（退化/稳定/renew 剔暂态/短跨度 NA/旧产物无 sent_at + stall.csv 解析）
...
## 12. 自部署实测复盘（2026-09-13 → 09-15，qwen3.8-27b 双环境四轮）

> 背景：pro5000 双卡 BF16 / H20 单卡 FP8 两个自部署环境连跑 r1（基础四格）→ 格② 100k 补测
> → r2（并发扩展 16–64）→ r3（思考 xhigh/low）四轮。本节只收**工具硌手点**；服务端侧发现
...
已完成：认证格式改造 → probe tool-call 检测 → trace 回放增强 → 硬编码其余项
        ...
        → 10.5 稳定性 soak（`duration_seconds` + `renew` 时长制原语 + 报告稳定性区：漂移/事故时间轴）
          【2026-09-16】
剩余：  无（第 10 节体系收敛完成）。
已清（2026-09-16）：12.1–12.13 全部落地（12.13 = 沙盘推演修复，证据见 docs/sandbox-review-2026-09-16.md）；5.9 起步值补样移出在途、不再跟踪。
...
| 5.7 会话续跑（trace 侧） | `renew: true` + `duration_seconds` 的 trace 回放侧用法 | 真实 soak 需求出现；**filler 侧已随 10.5 落地（2026-09-16），trace 侧仍留此处** |
