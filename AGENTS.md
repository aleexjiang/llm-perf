...
- **闭环 multiturn 目前是 barrier 齐射，没有错峰**（`runClosedRound` 的 `startBarrier` 让 level 个 worker 同时发首轮；`Concurrent` 配置段无任何爬坡字段）。turn2 起靠各 worker 完成时间差"自然退同步"，但那是随机结果、依赖服务端速度，不可复现，不能当受控变量。已拍板的方案是**内置指数爬坡发车**（默认开，`ramp: false` 可关；`ramp_factor` 默认 2）：批次触发挂在"上一批全部完成首轮"事件上，节奏自适应服务端速度；首轮失败即终止测试，中间轮失败继续（带止损机制）。**代码未实现，见 ROADMAP 5.7**——本条是设计决策不是现状描述。
